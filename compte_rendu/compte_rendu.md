...
# II. Tableaux comparatifs entre le cahier des charges et le projet final



# III. Diagrammes de Gantt
...
|-----------------------------------------------|-----------------------------------------------------------------------------------------------|
| ![Diagramme de Gantt](/images/1.png)          | ![Diagramme de Gantt](/images/1.png)                                                          |
| ![Diagramme de Gantt](/images/3.png)          | ![Diagramme de Gantt](/images/4.png)                                                          |
| ![Diagramme de Gantt](/images/5.png)          | ![Diagramme de Gantt](/images/6.png)                                                          |
| ![Diagramme de Gantt](/images/7.png)          | ![Diagramme de Gantt](/images/8.png)                                                          |
...
| Méthode                          | Description                                                                                                   |
|----------------------------------|---------------------------------------------------------------------------------------------------------------|
| __init__(self, msg_erreur:str=””)| Initialise les composants de l'interface : champs pour le login, mot de passe, IP du serveur, et bouton d'authentification. |
| creer_utilisateur(self)          | L'utilisateur est directement créé avec les informations saisies sans être stocké dans un attribut de l’IHM.  |
| quit(self)                       | Supprime la fenêtre d’authentification et quitte l’application.                                              |
...
| Méthode                                       | Description                                                                                   |
|-----------------------------------------------|-----------------------------------------------------------------------------------------------|
| __init__(self)                                | Initialise l’interface avec les informations de l’utilisateur et affiche les contacts disponibles. |
| lister_contacts(self) → None                 | Actualise la liste des contacts disponibles et gère les exceptions en cas d'erreurs.          |
| appeler_correspondant(self, correspondant:str)| Ferme la fenêtre des contacts et ouvre l’interface d’appel pour le correspondant sélectionné.  |
| ecouter_requete_appel(self) → None           | Écoute en arrière-plan les requêtes d’appels entrants et ouvre l’interface d’appel pour répondre. |
...
| ouvrir_ihm_appel(self, correspondant:str) → None | Ouvre l’interface d’appel pour un correspondant.                                            |
| quit(self) → None                            | Gère la fermeture de la fenêtre et la déconnexion de l’utilisateur.                           |
...
| Méthode                                          | Description                                                                                     |
|--------------------------------------------------|-------------------------------------------------------------------------------------------------|
| __init__(self)                                   | Initialise l’interface pour afficher l’état de l’appel et les boutons interactifs.             |
| envoyer_requetes_appel(self) → None             | Envoie une requête d’appel au serveur.                                                         |
| decrocher(self) → None                          | Accepte un appel entrant et démarre l’appel si le serveur l’autorise.                          |
| demarrer_appel(self, autorisation_de_demarrer_l_appel:bool, port_reception_voix_du_serveur:int) → None | Gère le démarrage de l’appel après validation par le serveur.                                  |
| raccrocher(self) → None                         | Informe le serveur de la fin de l’appel et ferme les flux audio.                               |
